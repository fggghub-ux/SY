(function() {
    ...
    function getCurrentViewportHeight() {
        ...
        return Math.max(0, Math.round(visualHeight || fallbackHeight));
    }

    function applyAppViewportHeight(options = {}) {
    ...
    function scheduleAppViewportSync(options = {}) {
        if (viewportSyncFrame) cancelAnimationFrame(viewportSyncFrame);
        viewportSyncFrame = requestAnimationFrame(() => {
            viewportSyncFrame = 0;
            applyAppViewportHeight(options);
        });

        clearTimeout(viewportSyncTimer);
        viewportSyncTimer = setTimeout(() => applyAppViewportHeight(options), 180);
    }

    window.syncAppViewportHeight = applyAppViewportHeight;
    applyAppViewportHeight();

    window.addEventListener('resize', () => scheduleAppViewportSync(), { passive: true });
    window.addEventListener('orientationchange', () => {
        scheduleAppViewportSync({ resetScroll: true });
        setTimeout(() => applyAppViewportHeight({ resetScroll: true }), 400);
    }, { passive: true });
