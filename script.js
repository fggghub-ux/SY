(function() {
    ...
    // ==========================================
    // 1.5 键盘弹起/收起全局适配 (visualViewport / resize)
    // ==========================================
    function setupKeyboardHandling() {
        ...
        function getSafeBottom() {
            ...
            dummy.style.cssText = 'position:fixed;bottom:0;height:env(safe-area-inset-bottom,0px);width:1px;pointer-events:none';
        ...
        setTimeout(() => { initialWindowHeight = window.innerHeight; }, 300);

        function applyViewport(vpHeight, offsetTop) {
            const kbHeight = Math.max(0, window.screen.height - vpHeight - offsetTop);
            app.style.height = vpHeight + 'px';
            app.style.position = 'fixed';
            app.style.top = offsetTop + 'px';
            app.style.left = '0';
            app.style.right = '0';
            
            app.setAttribute('data-kb-height', kbHeight);
            if (kbHeight > 80) {
        ...
        if (window.visualViewport) {
            function onVP() {
                ...
                applyViewport(vp.height, vp.offsetTop);
            }
            ...
            window.addEventListener('resize', function () {
                ...
                const kbH = Math.max(0, initialWindowHeight - h);
                app.style.height = h + 'px';
                app.setAttribute('data-kb-height', kbH);
                if (kbH > 80) {
                    app.classList.add('keyboard-open');
                } else {
                    app.classList.remove('keyboard-open');
        ...
        if (isIOS) {
            document.addEventListener('blur', function (e) {
                if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') {
                    setTimeout(function () {
                        if (window.visualViewport) {
                            applyViewport(window.visualViewport.height, window.visualViewport.offsetTop);
                        }
                        window.scrollTo(0, 0);
                    }, 100);
                }
